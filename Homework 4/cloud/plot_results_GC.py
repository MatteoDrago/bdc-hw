...
plt.yticks([i for i in range(len(numBlocks))], numBlocks)
...
for i in range(len(numBlocks)):
    plt.plot(k, objs[i,:], label='numBlock='+str(numBlocks[i]))
...
plt.subplot(3, 1, 2)
for i in range(len(numBlocks)):
    plt.plot(k, coreset_times[i,:], label='numBlock='+str(numBlocks[i]))
...
plt.subplot(3, 1, 3)
for i in range(len(numBlocks)):
    plt.plot(k, result_times[i,:], label='numBlock='+str(numBlocks[i]))
plt.legend(loc=2, prop={'size':8})
plt.grid(True)
plt.xticks(k)
plt.xlim(np.min(k), np.max(k))
